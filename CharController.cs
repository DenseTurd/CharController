...
public class CharController : MonoBehaviour
{
	...
	[SerializeField] float groundDrag = 15f;                                  // Drag to use when grounded with no horizontal input
	[SerializeField] float airDrag = 1f;                                      // Drag to use when grounded with no horizontal input
    ...
    void StartDash()
    {
        if (canDash)
        {
            ...
            Overseer.Instance.audioManager.DashJump();
        }
    }
    ...
    void RegularJump()
    {
        //if (canBounce) Debug.Log("Bounce jump");

        float force = canBounce ? jumpForce * 1.2f : jumpForce; // higher bounce jump
        grounded = false;
        dashing = false;
        ...
        Overseer.Instance.audioManager.RegularJump();
    }
    ...
    void SwingJump()
    {
        ...
        Overseer.Instance.audioManager.SwingJump();
    }
    ...
    void WallJump()
    {
        ...
        Overseer.Instance.audioManager.WallJump();
    }
    ...
    public void EnemyBounce()
    {
        ...
        Overseer.Instance.audioManager.BounceJump();
    }
    ...
    public void Volcano()
    {
        grounded = false;
        dashing = false;
        rb2D.AddForce(new Vector2(0f, jumpForce * 3f));
        SetVariableJumpHeightTimer();
        wallSlideCooldownTimer = wallSlideCooldownTime;

        poofParticles.Play();

        Overseer.Instance.audioManager.RegularJump(); // different audio call at some point??
    }
